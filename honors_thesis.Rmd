...
wordcloud(news.full.alt$Title,min.freq = 120,colors=brewer.pal(8, "Dark2"),random.color = TRUE,max.words = 1000)
```
...
news.cleaned.alt <- cbind(news.full.alt, title.sentimentLM.alt, title.sentimentHE.alt)
...
```{r}
#create alternate news source
#news.alt <- news.cleaned

#
#news.tibble <- as_tibble(news.cleaned)
#(news_tokens <- news.tibble %>%
...
#news_dtm <- news_tokens %>%
  #count(date, word) %>%
  #cast_dtm(document = date, term = word, value = n)
#removeSparseTerms(news_dtm, sparse = .99)
```
...
Neural net
```{r}
nnet.grid <- expand.grid(layer1 = 10, layer1 =20)
nnet.fit <- train(direction ~ ., 
                  data = GSPC.train,
                  method = "mlpML",
                  preProc = c("range"),
                  maxit = 1000, 
