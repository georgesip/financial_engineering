...
DEXCHUS <- read.csv("~/Desktop/Econ198/Data/DEXCHUS.csv", na.strings=".", stringsAsFactors=FALSE)
#Pre-processing
EX <- xts(DEXCHUS[, -1], order.by=as.Date(DEXCHUS$DATE))
names(EX) <- "USCHEX"
EX$USCHEX <-as.numeric(EX$USCHEX)
#Uneven rows, I matched them to the GSPC index in excel
USCHEX <- read.csv("~/Desktop/Econ198/code/Honors_thesis/DEXCHUS-edited.csv", stringsAsFactors=FALSE)
...
Processing to export as csv to check
```{r}
zooA = as.zoo(GSPC, order.by = row.names(GSPC))
write.zoo(GSPC, "gspc.csv", sep = ",")
write.csv(as.data.frame(GSPC),"test.csv")
```
