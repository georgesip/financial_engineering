...
GSPC.clean<-GSPC[ , !(names(GSPC) %in% drops)]
...
set.seed(123)
gspc.clean.df <- fortify.zoo(GSPC.clean)
...
GSPC.end <- floor(0.8*nrow(gspc.clean.df)) #select the first 80% of the data
GSPC.train <- gspc.clean.df[1:GSPC.end,] #assign the first 80% of the data to the train set
GSPC.test <- gspc.clean.df[(GSPC.end+1):nrow(gspc.clean.df),]
```
...
gspc.train.df <- as.data.frame(GSPC.train)
gspc.train.df$direction <- factor(gspc.train.df$direction)
...
Random Forest
```{r}
rf <- randomForest(direction~., data=gspc.clean.df, importance = TRUE)
rf$confusion
mean(rf$predicted == gspc.clean.df$direction)
```

<<<<<<< HEAD
```{r}
rf <- train(direction~., data=gspc.clean.df, method = "ranger")
rf$results
=======

```{r}
# set.seed(123)
# boost <- gbm(direction~., data=gspc.clean.df, distribution = "bernoulli")
# head(boost$fit)

>>>>>>> 178649591ba665c884128b08eef44916c063303e
```
