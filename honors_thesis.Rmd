...
news.cleaned.alt<-news.full.alt %>%
    distinct(Title, date, .keep_all = TRUE)
news.unclean <- news.unclean %>%
    distinct(Title, date, .keep_all = TRUE)
#remove NA's 
news.unclean[is.na(news.unclean)] = "unknown"
```
...
(out.desc <- with(
    news.unclean, 
    sentiment_by(
        get_sentences(news.unclean$textDiv), 
        date
    )
))
...
highlight(head(out))
first.desc <- head(news.unclean$textDiv)
highlight(head(out.desc))
```
